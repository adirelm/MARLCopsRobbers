...
Fail-CLOSED policy (it guards an email): contention refuses after a bounded wait and
a stale lockfile refuses with operator instructions — it is NEVER auto-deleted.
"""
...
from src.reporting.sentinel import LOCK
...
def _refuse_if_blocked(lockfile: Path, deadline: float) -> None:
    """Raise the clear operator error for a STALE lockfile or an expired wait."""
    with contextlib.suppress(FileNotFoundError):  # the holder may release between checks
        ...
        if age > _env_seconds("SENTINEL_LOCK_STALE_S", 300.0):
            raise RuntimeError(
                f"sentinel lockfile {lockfile} looks STALE (age {age:.0f}s) — a previous send "
                "process likely crashed holding it. Verify no send is running AND check the "
                "recipient inbox, then delete the lockfile manually and retry (never auto-removed)."
            )
...
def send_lock(sentinel: str | Path):
    ...
    ``SENTINEL_LOCK_STALE_S`` (default 300s) is presumed crashed and refused with
    operator instructions — this guards an email send, so it fails CLOSED.
    """
    ...
    with LOCK:
        ...
        while True:
            try:
                ...
                _refuse_if_blocked(lockfile, deadline)
                time.sleep(0.05)
        try:
            yield
        finally:
