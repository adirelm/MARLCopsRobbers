...
AFTER ``/new_sub_game`` reset the same session_id, so the idempotency cache is bound to
the EXACT request body — only an IDENTICAL re-POST replays a cached action; a differing
body for a cached tick recomputes and overwrites. The race is reproduced
...
def test_stale_request_finishing_after_the_void_reset_cannot_poison_the_new_run(cfg):
    ...
    try:
        ...
        agent.close()
    assert fresh == {"action": "left"}  # recomputed from the NEW body — NOT the stale cached "up"
    assert again == fresh and len(stub.calls) == 2  # identical re-POST replays; no third advance
