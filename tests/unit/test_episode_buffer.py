...
across more than one source). The episode-constant ``active`` mask gets its own
file (``test_episode_buffer_active.py``); shared builders live in
``_buffer_fixtures.py``.
"""
...
from tests.unit._buffer_fixtures import (
    _C,
    _CAP,
    _N,
    _N_ACTIONS,
...
def test_len_grows_then_caps_at_capacity():
    ...
    for i in range(_CAP + 3):
        buf.add_episode(make_episode(_T_MAX, fill=i), SourceTag.RANDOM)
...
def test_sample_returns_batched_shapes():
    ...
    buf = make_buffer()
    for i in range(_CAP):
        buf.add_episode(make_episode(_T_MAX, fill=i + 1), SourceTag.EXPERT)
    batch = buf.sample(3)
    ...
    assert batch["active"].shape == (3, _N)
...
def test_filled_mask_zeroes_padded_steps():
    ...
    buf = make_buffer()
    short = 2
    buf.add_episode(make_episode(short, fill=9.0), SourceTag.SELF_PLAY)
    batch = buf.sample(1)
...
def test_seeded_sampling_is_reproducible():
    ...
    buf_a = make_buffer(seed=42)
    buf_b = make_buffer(seed=42)
    for i in range(_CAP):
        ep = make_episode(_T_MAX, fill=i + 1)
...
def test_capacity_ring_overwrites_oldest():
    ...
    buf.add_episode(make_episode(_T_MAX, fill=1.0), SourceTag.EXPERT)
    buf.add_episode(make_episode(_T_MAX, fill=2.0), SourceTag.EXPERT)
    buf.add_episode(make_episode(_T_MAX, fill=3.0), SourceTag.EXPERT)  # evicts fill=1
    assert len(buf) == 2
...
def test_thief_buffer_single_agent_axis():
    ...
    buf = make_buffer(n_agents=1)
    buf.add_episode(make_episode(_T_MAX, n_agents=1, fill=1.0), SourceTag.RANDOM)
    batch = buf.sample(1)
...
def test_terminal_next_obs_survives_add_then_sample():
    """The terminal next-obs frame obs[real_t] (T+1 axis) round-trips (P4 unroll)."""
    buf = make_buffer(n_agents=1)
    ep = make_episode(_T_MAX, n_agents=1, fill=1.0)
    sentinel = 7.0
...
def test_four_source_tags_land_and_mix():
    ...
    for i, tag in enumerate(tags):
        buf.add_episode(make_episode(_T_MAX, fill=i + 1), tag)
    assert len(buf) == 4
