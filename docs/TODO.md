...
- [x] **T0.4 — `.env-example`, secrets policy, `.gitignore`** · _B_
  Tracked `.env-example` (names only, blank): `COP_MCP_TOKEN=`, `THIEF_MCP_TOKEN=`, `REFEREE_MCP_TOKEN=`, `MCP_PUBLIC_KEY=`, `MCP_PRIVATE_KEY=`, `REVOKED_TOKEN_JTIS=`, `MODEL_PATH=`, `GMAIL_SENDER=`, `GMAIL_APP_PASSWORD=`. (Deploy is Render via deploy/render.yaml — no platform API key; public URLs live in `config.cloud.*`.) Tracked repo-root `players.example.yaml` (placeholder role A — SOLO, PRD §1.3); real values in git-ignored repo-root `players.local.yaml`. Git-ignore `.env`, `.env.*` (keep `!.env-example`), `*.pem`, `*.key`, `credentials*.json`, `players.local.yaml`, `secrets/`, `results/.report_sent`, `results/reports/*.real.json`, `adrl-001-ex06.pdf`/`.docx`, `instructions/`, heavy `results/` artifacts.
  **DoD:** `git ls-files` shows zero secrets/PII; `scripts/check_secrets.py` + `scripts/check_pii.py` pass; cover-sheet path ignored.

- [x] **T0.5 — Docs-first PRD/PLAN/TODO + ADRs** · _A+B_
...
- [x] **T0.5b — Prompt log `docs/shared/PROMPTS.md` (§1.4 evidence)** · _A+B_
...
- [x] **T0.6 — Quality-gate tooling (RED first)** · _B_
...
- [x] **T0.7 — SDK single-entry skeleton** · _A_
...
- [x] **T0.8 — Repo sharing & branch** · _B_
  ...
  `bonus/minimax-q-baseline`) rather than a long-lived `assignment-6` branch; the repo is **PUBLIC** again
  since 2026-08-04. It was flipped private for the §9 window (so no opponent could train a
  best-response against our published weights — ANALYSIS §13) and restored once the match was
  played and agreed. `rmisegal` (Dr. Yoram Segal) also holds accepted read access from that period. Keep a `/tmp` clone to recover `.git` (MEMORY: Google-Drive sync
...
- [x] **T1.1 — Formal Dec-POMDP tuple + POSG caveat (README §7.1)** · _A_
...
- [x] **T1.2 — Action enum + masking (TDD RED→GREEN)** · _A_
...
- [x] **T1.3 — Grid + types primitives (TDD)** · _A_
...
- [x] **T1.4 — Reward model + Scorer, STRICTLY separate (TDD)** · _A_
...
- [x] **T1.5 — README §7 outline + figure manifest** · _A_
...
- [x] **T2.1 — Pure transition function (TDD)** · _A_
...
- [x] **T2.2 — Observation builder, fixed 5×5 padded (TDD)** · _A_
...
- [x] **T2.3 — Env class + train/exec split leak-test (TDD)** · _A_
...
- [x] **T2.4 — Curriculum scheduler (TDD)** · _A_
...
- [x] **T3.1 — Centralized episodic replay + four §5.2 data sources (TDD RED first)** · _A_
...
- [x] **T3.2 — Schemas + heuristics + dataset (TDD)** · _A_
...
- [x] **T3.3 — 2×2 pipeline smoke (integration)** · _A_
...
- [x] **T4.0a — Mixer ABC seam + VDN mixer (TDD)** · _A_ — _moved from P1 (#27): builds the network/training seam, belongs in P4 (matches PLAN P4, BRIEF §6 step 4)._
...
- [x] **T4.0b — Recurrent Q-net (TDD)** · _A_ — _moved from P1 (#27): the GRU Q-net is a training-network artifact, P4._
...
- [x] **T4.1 — QMIX mixer (TDD, monotone)** · _A_
...
- [x] **T4.3 — OLoRA (TDD, paper-faithful)** · _A_
...
- [x] **T4.4 — BC pretrain (Stage A)** · _A_
  `scripts/pretrain_bc.py` — generate Manhattan-heuristic `(o,a*)` pairs on 2×2/3×3 (~40k, seeded, `data/bc/<grid>.npz`, tiny seed subset vendored for CI), BC-train `CopNet`/`ThiefNet`, assert held-out **per-grid val-acc ≥ `bc.val_acc_gate_by_grid[min(H,W)]`** (`{2: 0.50, 3: 0.78}`; user-signed-off realistic gate — random floors cop 0.20/thief 0.25, Bayes ceilings cop ~0.635 @2×2 / ~0.823 @3×3) before OLoRA attach. The CI smoke asserts beats-random-by-a-margin + loss-decreases (a tiny vendored set under-trains the full gate).
  **DoD:** runs offline (no network); per-grid val-acc ≥ `gate_for(cfg, grid)` (full slow run); base checkpoints under `checkpoints/base/`; `L_BC = −E[log π(a*|o)]` cross-entropy.

- [x] **T4.5 — OLoRA attach (bundle + learner-injection seam + smoke)** · _A_
  **P4c (done here):** `src/marl/olora_bundle.py` (freeze base, QR-attach, save bundle `{base_sha, adapters(A,B), head_state}` with a load-time `base_sha` guard reconstructing the rebase from the base's INITIAL QR — never the trained adapters) + the backward-compatible `QmixLearner(..., net=None)` injection seam (optimizer net group filters `requires_grad`, so a wrapped net's frozen `W0`/`bias` are excluded) + a one-update smoke proving only `{A,B,head,mixer}` carry gradient.
  **DEFERRED to P4d (T4.5→T4.6 dependency):** `scripts/finetune_ctde.py` — the full OLoRA-wrapped CTDE curriculum-finetune **LOOP** (3×3→4×4→5×5) reuses the P4d `SelfPlayTrainer` + rollout + centralized replay, so the finetune orchestration lands with the trainer in P4d (kept as a `slow`-marked script, excluded from the fast default run).
  **DoD:** bundle round-trips with `base_sha` match; a tampered `base_sha` raises `ValueError`; an OLoRA-wrapped learner runs one `update` and only `{A,B,head,mixer}` have grad (`W0`/`bias` frozen, zero/None grad); `net=None` still builds + trains the full dense net (P4b unchanged). [P4d] finetune emits loss + learning curves across stages; one frozen encoder spans the whole ladder (pad-to-5 verified).

- [x] **T4.6 — Self-play orchestration (TDD)** · _A_
...
- [x] **T4.7 — Seeded sweep harness → JSONL** · _A_
...
- [x] **T4.8 — SDK training entry + thin scripts** · _A_
...
- [x] **T5.1 — `mcp:` config + token placeholders** · _B_
...
- [x] **T5.2 — Wire schemas (TDD RED first)** · _B_
...
- [x] **T5.3 — `schemas.py` + `actions.py` (single wire-JSON source)** · _B_
...
- [x] **T5.4 — Auth module (local→cloud, revocable)** · _B_
...
- [x] **T5.5 — Private agent runtime + recurrent session state (DRY)** · _B_
...
- [x] **T5.8 — Referee / MatchRunner** · _B_
...
- [x] **T6.1 — `run_match.py` orchestrator** · _B_
...
- [x] **T6.2 — Match integration test** · _B_
...
- [x] **T6.3 — Assemble §3.5 JSON (no send)** · _B_
...
- [x] **T7.1 — `SpectatorFrame` + SDK session (TDD)** · _B_
...
- [x] **T7.2 — Headless test harness** · _B_
...
- [x] **T7.3 — Transform + palette (TDD)** · _B_
...
- [x] **T7.4 — Grid view + HUD + score view (TDD)** · _B_
...
- [x] **T7.5 — Input map + state client + app (TDD)** · _B_
...
- [x] **T7.6 — Launcher + screenshot automation** · _B_
...
- [x] **T7.7 — GUI architecture gates** · _B_
...
- [x] **T7.8 — `docs/UX.md` Nielsen heuristics (§10 REQUIRED)** · _B_
...
- [x] **T8.5 — Render fallback (tested, not theoretical)** · _B_
...
- [x] **T8.6 — Cloud GUI spectator path** · _B_
...
- [x] **T9.1 — Report config + secrets + players** · _B_
...
- [x] **T9.2 — Schema + validators (TDD RED first)** · _B_
...
- [x] **T9.3 — Collector + builder + players + redact + idempotency** · _B_
...
- [x] **T9.4 — Mailer (TDD, smtplib default)** · _B_
...
- [x] **T9.5 — SDK send + Cop trigger (idempotent)** · _B_
...
- [x] **T9.6 — Report CI gates + evidence** · _B_
...
- [x] **T10.1 — Aggregator + plots + make_figures** · _A_
...
- [x] **T10.4 — README §7.2 critical analysis (4 sub-parts)** · _A_
...
- [x] **T10.5 — README §7.3 figures + risk register + remaining docs** · _A_
...
- [x] **T10.7 — §9 analysis notebook (SDK-only)** · _A_
...
- [x] **T10.8 — §11 cost analysis (`docs/COST_ANALYSIS.md`)** · _A_
...
- [x] **T10.9 — §13 ISO/IEC 25010 quality (`docs/QUALITY.md`)** · _A_
...
- [x] **T11.1 — Run software-excellence-v3 audit** · _A+B_
...
- [x] **T11.2 — Final gate sweep** · _A+B_
...
- [x] **T11.4 — Per-module test matrix (V3 §6)** · _A+B_
...
- [x] **T11.3 — Self-grade + freeze + cover sheet** · _A+B_
...
- [x] **TB.3 — Bonus match** · _B_ — **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a)
  Partner `biu-azri` identified; the 11-row pre-game agreement was frozen in writing before either side played; both policies frozen (no training during the match); 6 alternating-role sub-games played (1-3 `adrl-001` Cop / 4-6 swapped) over the neutral raw-position wire protocol with our referee hosting; drafts byte-compared by exchanging a canonical sha256 of `sub_games`/`totals_by_group`/`bonus_claim` (1261 bytes, identical both sides) BEFORE either sent; BOTH groups emailed their bonus report.
  **DoD MET:** both groups emailed valid bonus reports with `mutual_agreement:true`, so the +10/+7 is claimable rather than both-zero; README §9 "Bonus Game" section is present with opponent, final score, claim and the 18 replay screenshots — the not-attempted branch does not apply.
...
| 8 | **Version starts `1.00`** | started `1.0.0` → `1.1.0` (L11 §5 Minimax-Q bonus) → now `1.2.0` (V3 breadth; `1.1.1` was the post-audit hardening — the §9 inter-group "P-bonus" is a SEPARATE track, now COMPLETE): `src/__init__.py::__version__` == `config.version` == `pyproject` (ADR-0011) | `tests/unit/test_config_loader.py` (see the implementation note above) |
