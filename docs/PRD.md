...
| **P3** | Minimal pipeline | End-to-end collect→train→export pipeline + **throwaway tabular 2×2 smoke** (Table 2 row 1, deleted in P4) + dataset/replay machinery (centralized episodic replay buffer, BC dataset/obs-encoder, Manhattan-heuristic oracles). **BC training (val-acc ≥0.9) is P4/T4.4 — NOT P3.** | 2×2 reaches **pursuit-optimal capture from every 2×2 start over all `training.seeds`** (thief flees ⇒ naive min-Manhattan wrong; a zeroed Q fails the gate); pipeline runs NaN-free; headless `render_state()` god-view JSON + minimal sub-game JSON (GUI shot → P7) |
