...
- **FR-MCP-6 (revocable bearer auth).** All endpoints require a revocable bearer token; invalid/revoked → HTTP 401.
  ...
  - **Evidence:** `tests/integration/test_mcp_servers.py` (live 200/401) + `tests/unit/test_jwt_auth.py` (expiry/revoke/audience); the Stage-2 screenshot (`cloud_auth_401.png`) is account-gated, NOT captured (README §6). ADRs: 0011/0012 (PLAN §6; D-draft IDs consolidated — see the §6 note).
