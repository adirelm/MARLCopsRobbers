...
| **L3** | **Reward:** **potential-based shaping** (provably policy-invariant, Ng 1999: `F = γΦ(s') − Φ(s)`, `Φ = −Manhattan(cop,thief)`; `−1/step` + distance-to-thief potential) during **training only**, **toggled off at eval**. The official §3.4 scoreboard (cop_win 20 / thief_win 10 / cop_loss 5 / thief_loss 5) is computed **separately** for reports. | §3.4, §7 |
...
| **S** | global state (**train-only**) | `(cop_pos:(r,c), thief_pos:(r,c), barriers:set[(r,c)], barriers_used:int, step:int, terminal:bool)`. Encoded for the centralized critic as `3·G·G + 2` floats (G=5 → S=77). Reachable ONLY via `env.state()`, **never** via `step()`. |
| **A** | joint action `A_cop × A_thief` | `\|A_cop\| = 5 {UP,DOWN,LEFT,RIGHT,PLACE_BARRIER}`; `\|A_thief\| = 4 {UP,DOWN,LEFT,RIGHT}`. `STAY`/`NOOP` exists in the enum but is config-gated **off** by default (brief rule: both move one cell). `PLACE_BARRIER` is cop-only, masked when `barriers_left == 0`. |
| **T** | transition `T(s'│s,ā)` | Deterministic core: **simultaneous** joint-move resolution; barriers & edges impassable to both; barrier placement consumes the move; optional `transition_noise` (default 0.0). The Thief is folded into `T` from any single cop learner's view → **non-stationarity** (§2.1). |
| **R** | reward | RL signal = config-driven shaped reward (L3); shared global scalar in `dec_pomdp` mode, per-agent in `posg` mode. The official §3.4 scoreboard is a **separate** `Scorer`, never the training signal. |
| **Ω** | observation space | Per-agent egocentric Manhattan-radius window. |
| **O** | observation function `O(ō│s',ā)` | Deterministic crop of S to radius-`r` cells around agent `i`; `O = 1` for the true window; optional `obs_noise` flips the opponent-visible channel. |
...
(eq 2≡4) IQL target (baseline): y_i = r_i + γ(1-d) max_{a'_i} Q_i(o'_i, a'_i ; θ_i⁻)
...
| **K5 Reproducibility** | All 6 figures regenerate from `uv run python -m src.results.make_figures` with zero README/code drift | `results/runs/*.jsonl` + `experiment_manifest.json` (seeds, config hash, git commit) | F1–F6 + manifest |
...
- **FR-ENV-1 (eq 1 formalism).** A custom **grid-agnostic** `CopsRobbersEnv` implements the formal Dec-POMDP tuple ⟨N,S,A,T,R,Ω,O,γ⟩ (eq 1, cite [1]).
  - **AC:** a unit test asserts `\|N\|=2`, per-role action enums (`\|A_cop\|=5`, `\|A_thief\|=4`), and `γ` read from config; README §7.1 maps each tuple symbol to code.
...
- **FR-ENV-3 (actions).** Per-agent action set with `PLACE_BARRIER` cop-only; invalid actions map to NOOP, are logged, and still consume the step.
  - **AC:** action-mask tests for thief-`PLACE_BARRIER`-masked, out-of-bounds, barrier-blocked, and budget-exhausted cases.
...
- **FR-ENV-4 (transition & barriers, §3.3).** Transition is **simultaneous-resolution** (`env.move_resolution:simultaneous`); barriers and board edges are impassable to **both** agents; a barrier is placed on the cop's current cell and consumes the move; ≤`game.max_barriers` (5) per sub-game.
  - **AC:** tests for edge==wall (stay-in-place), barrier blocks entry for both agents, barrier-budget cap, placement increments `step_count`.
...
- **FR-ENV-5 (capture & termination, §3.2).** Capture occurs when cop and thief share a cell after the simultaneous update, **including a one-step swap** (`env.capture_on_swap:true`). Termination = capture OR `step ≥ game.max_moves` (25). Treating a simultaneous cop↔thief cell-swap as a capture is a **deliberate, documented architect game-rule call** (BRIEF §3.2 is silent on swap-through under simultaneous resolution; counting it as capture keeps the pursuit well-posed) — a human-decided, non-delegable rule clarification (CLAUDE.md §1.4).
  - **AC:** tests for shared-cell capture, swap=capture, timeout at exactly `game.max_moves`, `winner ∈ {cop, thief}`.
  - **Evidence:** `tests/env/test_transition.py`; ADR-001 (move resolution + swap-as-capture clarification).
- **FR-ENV-6 (reward decoupling, L3).** The RL reward is a config-driven shaped signal with **potential-based, grid-normalized** distance shaping (`F = γΦ(s') − Φ(s)`, Ng 1999); per-agent POSG rewards are also computed. The Table-1 game points (20/10/5/5) are produced by a **separate** `Scorer` and never reused as RL reward; shaping is toggled off at eval.
  - **AC:** a test asserts the potential-shaping identity; a test asserts `RewardModel` and `Scorer` are distinct modules; a test asserts `dec_pomdp` reward-dict entries are equal.
...
- **FR-ENV-7 (partial observation Ω/O).** A fixed `5×5` egocentric image with `env.obs_channels`=5 channels (self, other_visible, barrier, out_of_bounds, time_norm) + an `env.obs_scalars`=6 scalar vector, **padded across all curriculum stages** to footprint `2·env.view_radius_max+1`=5 so one network spans 2×2→5×5 (enables OLoRA). View radius comes from `env.view_radius_by_grid` (auto `r = max(0, ⌈min(H,W)/2⌉ − 1)`: 2×2:0, 3×3:1, 4×4:1, 5×5:2) with config override; the opponent channel is zeroed beyond Manhattan radius.
  - **AC:** tests assert obs image shape `(5,5,5)` at 2×2/3×3/4×4/5×5 and a length-6 scalar vector; `other_visible = 0` when out of radius.
  - **Evidence:** `tests/env/test_observation.py`.
...
- **FR-ALG-1 (value decomposition, L2).** Train a value-based CTDE MARL agent for the Cop side using value decomposition. **QMIX is the graded PRIMARY** (`algo.name:qmix`, monotonic mixer eq 7; L2/ADR-D10-A); **VDN is the ablation arm** (eq 6, selected via `algo.mixer.type:vdn`); **IQL is the §7.2 baseline** (`algo.baseline:iql`). The arm is selected via `algo.name` ∈ {qmix, vdn, iql}.
  - **AC:** with `algo.mixer.type=vdn` a unit test asserts `Q_tot == Σ_{i∈cop} Q_i` over the cop team; with `algo.name=qmix` a numeric test asserts `∂Q_tot/∂Q_i ≥ 0` and IGM-consistency (eq 5: decentralized greedy joint action == argmax of mixer output).
  - **Evidence:** `tests/unit/test_mixer.py`.
...
- **FR-ALG-4 (shared net + swappable mixer seam, DRY).** All three algorithms share ONE `RecurrentQNet` (GRU, eq 8) and ONE centralized episode replay buffer; only the `Mixer` ABC changes.
  - **AC:** a single `agent_net.py` + buffer is imported by the iql/vdn/qmix paths (no duplication).
...
- **FR-ALG-8 (losses & optimization).** Centralized Double-DQN target (`algo.double_q:true`) with masked Huber loss (`algo.huber_delta`=1.0); one AdamW per learner with param groups (`algo.lr_agent`=3e-4, `algo.lr_mixer`=5e-4); hard target sync every `algo.target_update_interval` (200; `algo.target_mode` config-selectable to soft with `algo.tau`).
...
- **FR-OLoRA-1 (locally pre-trained encoders).** Produce locally pre-trained role encoders (CopNet, ThiefNet) via **behavior cloning** on Manhattan-heuristic data over small grids (2×2/3×3) — the only honest reading of "pre-trained model" that satisfies "training MUST be local".
  - **AC:** `scripts/pretrain_bc.py` runs offline; held-out heuristic-label validation accuracy ≥ `bc.val_acc_gate` (0.9) before OLoRA attach; base checkpoints saved under `checkpoints/base/`.
...
- **FR-MCP-3 (local-obs-only execution, single canonical tool contract).** Each server acts using ONLY its referee-supplied local Manhattan-radius observation; no global state and no opponent location enter `request_move`. ONE tool contract is used for BOTH localhost Stage 1 and cloud Stage 2 — the canonical tools are `request_move`, `reveal_location`, `query_opponent`, `new_sub_game`, `send_final_report` (in `src/mcp/`); there is NO divergent `propose_action`/`commit_turn` fork.
  - **AC:** a unit test asserts the `request_move` input is exactly the `Observation` schema; a test asserts the `query_opponent`/`reveal_location` result is **never** passed into the next `request_move`; a test asserts the same tool names are registered on the local and cloud server builds.
...
- **FR-MCP-7 (full local run, Stage 1).** The system runs end-to-end on localhost (two ports) producing exactly 6 valid sub-games before any cloud deploy.
  - **AC:** `run_match.py --stage local` completes 6 valid sub-games + emits a referee trace.
...
- **FR-CLOUD-5 (cloud match emits JSON, does NOT send).** A full 6-sub-game match runs over the two cloud URLs and emits the §3.5 JSON body; it does NOT send the email (that is §6 step 9).
  - **AC:** `cloud_match.py` produces a valid JSON artifact and sends no mail.
...
- **FR-ANL-7 (reproducible figures).** All figures regenerate from one command `uv run python -m src.results.make_figures` reading `results/runs/*.jsonl`, with a `results/experiment_manifest.json` recording seeds, config hashes, git commit, run IDs.
  - **AC:** rerunning `make_figures` reproduces the committed figures; the manifest is complete.
...
| **NFR-11** | **Docs-first SDLC (PRD/PLAN/TODO + ADRs)** | this PRD + `docs/PLAN.md` (C4/UML/deployment + ADRs **0001–0014**) + `docs/TODO.md` exist with human §1.4 sign-off before feature code | `test_required_docs_present.py` + ADR-count check (0001–0014) |
...
- **NFR-13 (reproducibility).** Fixed `training.seeds` `[7,17,37,71,107]`; single seeded RNG in the replay buffer; `torch.manual_seed` per learner from each `training.seeds` entry; `results/experiment_manifest.json` (`paths.experiment_manifest`) pins seeds/config-hash/git-commit so README numbers cannot drift from code (R8).
...
| **P3** | Minimal pipeline | End-to-end train→eval on 2×2 (Table 2 row 1); BC pretrain on 2×2/3×3 (val-acc ≥0.9) | 2×2 reaches optimal; pipeline green; first GUI shot + JSON sub-game |
...
| **P11** | gate | `software-excellence-v3` audit; self-grade; freeze 24h pre-deadline; produce git-ignored cover sheet (Moodle only) | all hard gates ✅; audit doc with real evidence; ADR count 0001–0014; `test_required_docs_present.py` green (COST_ANALYSIS/QUALITY/UX present); never assert the git-ignored cover sheet in a test |
...
- **Env → Algorithm:** env publishes obs/state dims, the view-radius schedule, channel layouts, action enums, masking semantics, and a `state()` method distinct from `step()`; the centralized replay-tuple `(obs_dict, joint_action, global_reward, reward_i, next_obs_dict, terminated, GlobalState)` lets CTDE stabilize against non-stationarity. Net input dim = padded-max `W_v` (5×5) so one net spans the curriculum + OLoRA.
...
- **OD4 (P4):** `mixer.include_opponent_utility` default false (clean per-team QMIX + honest N=1 caveat) vs true (genuinely 2-input mixer, +2 train-only nets) for stronger §7.2 evidence.
