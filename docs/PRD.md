...
> **Group:** `adrl-001` · **Version:** `1.2.0` · **Recipient of record:** `rmisegal+marl@gmail.com`
...
- ~~Running the actual §9 **inter-group bonus match**~~ — this was a non-goal while A6 was in flight, and it has since been RETIRED as one: the match was **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a). It still counts toward the **final project**, not A6. The serializer + schema + agreement/wire spec (`src/reporting/bonus.py`, `bonus_send.py`, `docs/schema/bonus.schema.json`, `docs/interfaces/intergroup_mcp.md` — FR-ANL-9) are BUILT and were what the match ran on.
...
|---|---|---|---|
| **K1 Credit-assignment / decomposition (4×4 2-cop)** | Credit-assignment studied on the **4×4 2-cop scenario** (`env.curriculum.num_cops_by_stage[2]=2`) — the genuinely multi-agent mixer, NOT the degenerate 1-cop 5×5 (trivial scalar gain, §7.2 L4). **Honest empirical finding (reported faithfully, README §7.2): the `QMIX ≥ VDN ≥ IQL` hypothesis is FALSIFIED at the 50-round budget** — `VDN (0.84) ≥ IQL (0.82) > QMIX (0.63±0.10)`; QMIX's monotonic hypernetwork is more expressive but harder to train (R1 instability), so the simpler decompositions win at this bounded budget | seeded sweep on the 4×4 2-cop stage, identical nets/replay/seeds/curriculum across arms | F5 `results/figures/baseline_comparison.png` (4×4 2-cop panel) |
| **K2 Cop policy quality (4×4 focus stage)** | Honest result at the **4×4 focus stage** (held-out seeds, shaping off): the best CTDE arm (VDN, **0.84**) clears the IQL baseline (**0.82**); the graded-primary QMIX **lags** IQL at this 50-round budget (**0.63**, the R1 instability — see K1). The graded 1-cop **5×5 IS now swept** (the §5.1 final test; F1/F2 learning+loss curves render there): QMIX **0.72** > IQL/VDN **0.61** (1-cop, so VDN's sum-decomposition = IQL). K2-as-originally-specified (strictly-above-IQL on 5×5) HOLDS for the graded QMIX primary | held-out 4×4 capture-rate, shaping off | F5 (4×4 focus panel) |
...
| **K6 Cloud comms** | One shared `trace_id` appears in BOTH servers' logs for cop↔thief calls; 401 without token; revoke demo | cloud smoke run + redacted captures | **CAPTURED LIVE (2026-07-22)**: `results/figures/mcp_comms_cloud.png` (shared trace across BOTH cloud servers) + `results/figures/cloud_auth.png` (200/401/401/401 matrix incl. wrong-audience); localhost F4 + F4b also shipped |
| **K7 V3 hard gates** | All green (≤150 LOC/file, ≥85% cov, ruff 0, no-hardcode, no-secrets, uv-only, single-SDK-entry, version 1.2.0) | CI pipeline + `software-excellence-v3` audit | `instructions/assignment-6/submission_guidelines_audit.md` |
...
- **FR-ALG-1 (value decomposition, L2).** Train a value-based CTDE MARL agent for the Cop side using value decomposition. **QMIX is the graded PRIMARY** (`algo.name:qmix`, monotonic mixer eq 7; L2/ADR-D10-A); **VDN is the ablation arm** (eq 6, selected via `algo.mixer.type:vdn`); **IQL is the §7.2 baseline** (`algo.name:iql`). The arm is selected via `algo.name` ∈ {qmix, vdn, iql}.
...
- **FR-MCP-6 (revocable bearer auth).** All endpoints require a revocable bearer token; invalid/revoked → HTTP 401.
  ...
  - **Evidence:** `results/figures/cloud_auth.png` — LIVE against both public cloud endpoints (valid→200; bad/missing/wrong-audience→401); plus `tests/integration/test_mcp_servers.py` + `tests/unit/test_jwt_auth.py`. ADRs: 0011/0012 (PLAN §6; D-draft IDs consolidated — see the §6 note).
...
- **FR-CLOUD-1 (public deploy).** Both MCP servers are LIVE on a public cloud host with HTTPS URLs `https://adrl-001-<role>.onrender.com/mcp` (**Render**; Prefect Horizon / FastMCP Cloud was the original plan but its free tier forces platform OAuth in front of the app — reversed 2026-07-22).
...
- **FR-CLOUD-4 (revocable, idempotent, resilient).** Tokens are revocable (jti deny-list primary; key rotation hard lever) and externally minted with a short TTL verified via the `exp` claim — no token is minted in-repo (deploy is inference-only; token VALUES live in `.env`, see ADR-0012); the client uses `mcp.client.timeout_s`=10, `mcp.client.max_retries`=3 with `backoff_s`=0.5, and a pre-game warm-up ping (`prewarm_ping:true`); `request_move` is idempotent per `(session_id, tick)`.
  ...
  - **Evidence:** `tests/unit/test_jwt_auth.py` (revoked jti → rejected, fresh token → accepted) + the live revoke check on the deployed stack (recorded in `results/figures/cloud_auth.png`). ADR-0012 (PLAN §6).
- **FR-CLOUD-5 (cloud match emits JSON, does NOT send).** A full 6-sub-game match runs over the two cloud URLs and emits the §3.5 JSON body; it does NOT send the email (that is §6 step 9).
  - **AC:** a match over the two cloud URLs runs and emits the §3.5 JSON without sending mail — **DONE LIVE (2026-07-22)**: a FULL 6-sub-game match ran between the two Render servers over the public internet (F4c, traces sg-0…sg-5, cop 30 – thief 60) and its schema-valid report body is committed at `results/subgames/cloud_match_5x5.redacted.json` (no mail sent). The as-built local equivalents ARE shipped and verified: `scripts/run_match.py` (full local match + dry-run ack) and `scripts/serve_match_http.py` (over real HTTP), with the committed artifact `results/subgames/full_match_5x5.redacted.json`.
...
- **FR-GUI-3 (deterministic multi-size screenshots, §7.3c).** Automated, deterministic GUI screenshots at 2×2/3×3/4×4/5×5, each showing cop + thief + the HUD on a mid-game board; barrier rendering is evidenced by the dedicated `state_barriers.png` (the heuristic agents only navigate around barriers and never place them, so a hand-set demo state exercises the real barrier draw path — stated in its README caption).
  - **AC (as built):** `uv run --extra gui python scripts/capture_screens.py` deterministically produces `results/screenshots/grid_{2,3,4,5}x{n}.png` + `state_{terminal,barriers,view_radius}.png` headlessly (`SDL_VIDEODRIVER=dummy`); token/HUD/barrier RENDERING is pixel- and plan-verified on rendered frames by `tests/integration/test_gui_render.py` + `tests/unit/test_draw_plan.py` (cop/thief cell-centre pixels, barrier fill ops, HUD lines, the 720×560 HUD-reservation guard) — the committed PNGs are captured artifacts, regenerated by the same seeded script rather than asserted by tests.
...
- **FR-RPT-1 (single end-of-game send).** After the 6th **valid** sub-game, the Cop process sends exactly ONE email to `gmail.to` (`rmisegal+marl@gmail.com`) with a structured subject and a JSON body covering all 6 sub-games. The egress path is `sdk.send_final_report` → `send.send_report` → §5 gatekeeper → `GmailMailer`. The **cop's `send_final_report` MCP tool is not that path**: it validates the body at the server boundary and acks `{sent: false, dry_run: true}` unconditionally (`src/mcp/server_builder.py:175`), so no SMTP dial can originate from an MCP call. Two separate things share the name deliberately — the tool is the in-protocol *validation + ack*, the SDK method is the *egress*.
...
- **FR-ANL-4 (empirical IQL-vs-CTDE baseline, F5).** The §7.2 baseline is EMPIRICAL (figure F5), using identical nets/replay/seeds/curriculum so the mixer is the only difference. The credit-assignment study (K1) on the **4×4 2-cop** panel (genuinely multi-agent mixer) shows the **honest empirical ordering `VDN ≥ IQL > QMIX`** — the `QMIX ≥ VDN ≥ IQL` hypothesis is FALSIFIED at the 50-round budget (QMIX's monotonic mixer destabilizes, R1; more-expressive-but-harder-to-train), reported faithfully per README §7.2. The graded 1-cop **5×5 stage IS now swept** (the §5.1 final test drives F1/F2); F5's 3-way comparison stays on the **4×4 2-cop** panel where the algorithms actually separate (at 1-cop 5×5 VDN=IQL exactly); the K2 cop-quality result + the N=1 trivial-decomposition discussion are therefore scoped to 4×4 / prose (§7.2), NOT a 5×5 figure.
  - **AC:** F5 shows IQL vs VDN vs QMIX with final capture-rate + cross-seed variance on the **4×4 2-cop focus panel** (5×5 is swept as the §5.1 final test but is 1-cop, where VDN=IQL, so the 3-way comparison stays at 4×4).
...
- **FR-ANL-9 (§9 bonus interface — BUILT; match = coordination dependency).** The `bonus_game` JSON serializer + validation (`src/reporting/bonus.py`: §9.1 role alternation, derived `totals_by_group`, derived §9.2 `bonus_claim` 10/7/5-5), the gated idempotent send with the §9.4 subject + dual-block redaction (`src/reporting/bonus_send.py`), the structural schema (`docs/schema/bonus.schema.json`), and the inter-group rules-agreement + neutral wire protocol (`docs/interfaces/intergroup_mcp.md`) are implemented and tested (`tests/unit/test_reporting_bonus.py`, `test_bonus_send.py`). The bonus counts toward the final project. The match is no longer a pending dependency — it was **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a).
  - **AC:** the bonus schema includes `report_type:"bonus_game"`, `groups{group_1,group_2}`, `github_repo_group_1/2`, `students_group_1/2`, `totals_by_group`, `bonus_claim`, `mutual_agreement`; each bonus `sub_games[]` entry **KEEPS the §3.5 keys** `{id,start,end,moves,winner,scores{cop,thief}}` **AND adds** `cop_group` + `thief_group` (per ex06 §9.4) — the §3.5 fields are NOT dropped; the §3.5 email serializer is reusable for the bonus variant; the bonus subject string follows the brief's §9.4 example **verbatim** — `[MARL Bonus Game] {group_1} vs {group_2} – Final Report` (`config.gmail.bonus_subject_template`). An earlier draft of this AC specified a house style (`[MARL Cops&Robbers BONUS] … | Final {num_games} sub-games | …`); that was **superseded** once ex06 §9.4 turned out to give a literal example, since matching the lecturer's own shape beats internal consistency here; the interface spec is published in `docs/` and marked as a **coordination dependency, not buildable solo**.
...
- **FR-API-1 (§5 single ApiGatekeeper — flips N/A→REQUIRED).** A6 makes real runtime HTTP (peer-MCP) + Gmail calls, so **ALL outbound egress** routes through one central `ApiGatekeeper` (`src/api/gatekeeper.py`) exposing `execute(call)` AND `get_queue_status()`. It applies a **per-channel token-bucket** read from the versioned, git-tracked `config/rate_limits.json` (channels `peer_mcp` 120/min burst 10, `gmail` 5/min burst 1), a **FIFO overflow queue** (`overflow_policy:"fifo_queue"`, `on_overflow:"enqueue"`, `max_queue:256` — **no crash on burst**), and logs every call (`log_all_calls:true`). When the FIFO overflow queue **itself** is full (depth == `max_queue`), the gatekeeper **rejects the call with an error and logs it** (it does **not** crash and does **not** silently drop) — the bounded queue degrades gracefully. No secrets live in `rate_limits.json` (tokens in `.env`).
  - **AC:** `test_egress_via_gatekeeper.py` asserts every peer-MCP/Gmail call path goes through `ApiGatekeeper.execute`; a burst test exceeding `burst` enqueues (does not raise) and `get_queue_status()` reflects the depth; a test filling the queue to `max_queue`+1 asserts the gatekeeper **returns/raises a handled rejection error AND emits a log line** (no crash, no silent drop); limits are read from `config/rate_limits.json` (zero hardcoded); a grep asserts no direct `httpx`/`smtplib` egress bypasses the gatekeeper.
  - **Evidence:** `tests/architecture/test_egress_via_gatekeeper.py`; `config/rate_limits.json`; ADR-0009; NFR-9.
- **FR-RES-1 (§9 sensitivity analysis — distinct from the IQL/VDN/QMIX ablation).** A controlled **single-parameter sweep** documents the swept value's effect on capture-rate with a figure. The pinned swept parameter is **`env.view_radius_by_grid`** (other candidates `olora.rank` / `selfplay.window_k` are acceptable, all other config keys held fixed). **`reward.distance_weight` is a poor sweep parameter** — potential-based shaping is policy-invariant in the limit (Ng 1999, single-agent asymptotic; multi-agent transfer per Devlin & Kudenko 2011), so at convergence it should not move the policy; at our finite 50-round budget it mainly measures optimization speed, a weaker §9 story than an observability knob. This is the §9 **sensitivity** requirement — the algorithm comparison (FR-ANL-4) is an **ablation**, NOT the sensitivity sweep.
...
- **FR-RES-2 (§9 analysis notebook — SDK-only).** Ship `notebooks/analysis.ipynb` that consumes the **`MarlSDK` seam** — no direct `env`/`marl`/`mcp`/`services`/`learner` imports. The config loader is permitted because `MarlSDK(cfg)` REQUIRES a config dict, so loading one is a precondition of constructing the SDK rather than a way around it; the read-only results aggregator is permitted because it reads committed artifacts and holds no business logic. Renders the LaTeX Dec-POMDP/QMIX equations (eq 1, eq 7), the F1–F6 figures, and the `[1]`–`[11]` citations.
  - **AC:** the notebook imports `src.sdk` and NONE of `src.marl` / `src.mcp` / `src.services` / `src.learn`; it regenerates/embeds the §7.3 figures and the formal equations. Evidence: `tests/architecture/test_required_docs_present.py::test_analysis_notebook_imports_sdk_only` — the gate that actually enforces this. (An earlier version of this AC said "imports only `src.sdk`" and pointed at `test_import_boundary.py`, which contains no notebook assertion at all; a cross-model audit caught both. The doc-freshness gate only checks that a cited test FILE exists, not that it covers the claim.)
  - **Evidence:** `notebooks/analysis.ipynb`; NFR-7.
- **FR-COST-1 (§11 cost analysis).** Ship `docs/COST_ANALYSIS.md` with (a) the **AI-assisted-development token-cost breakdown** (input/output token counts, $/1M per model, total), (b) the **RL training-compute envelope** (episodes × stages × wall-clock, local-only — `selfplay.rounds`(50) × `selfplay.episodes_per_round`(100) = 5000 episodes/stage × `env.curriculum.stages` (4) × seeds (5)), and (c) optimization notes (OLoRA param reduction, local-only training, caching).
...
| **NFR-8** | **Version starts at stated version** | `__version__ = "1.2.0"` in `src/__init__.py` == `config.version` (started V3 "1.00"; 1.1.0 = Minimax-Q bonus, 1.1.1 = post-audit hardening; 1.2.0 = V3 breadth (§9.3 charts, §12 seams, §14 exports, §16 blocks)) | `tests/unit/test_config_loader.py`; ADR-0011 |
| **NFR-9** | **§5 External-API governance (flips N/A→REQUIRED)** | A6 makes real runtime HTTP (peer MCP) + Gmail calls → `src/api/gatekeeper.py::ApiGatekeeper` (`execute` + `get_queue_status`; per-channel token-bucket from versioned `config/rate_limits.json`; FIFO overflow queue `max_queue:256`, no crash; all calls logged) — see FR-API-1 | `tests/architecture/test_egress_via_gatekeeper.py`; ADR-0009 |
...
| **P8** | Cloud-MCP | Render deploy of both servers (deploy/render.yaml); public URLs + revocable JWT (Prefect Horizon was the original plan, reversed — OAuth clash) | distributed match runs (≥1 valid cloud sub-game; shared trace_id); cloud auth/revoke proofs captured (upside per R2) |
...
| **P-bonus** | §9 post-A6 | Inter-group interface spec published; `bonus_report` serializer (keeps §3.5 sub-game keys + adds `cop_group`/`thief_group`; role alternation 1–3/4–6; scoring +10/+7/+5; FR-ANL-9/FR-ANL-10); match **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a) | schema + spec present; match COMPLETED — both groups emailed valid bonus reports with `mutual_agreement:true`, so it is claimed, not marked not-attempted |
...
- **MCP/Cloud → Report:** the match runners emit the §3.5 JSON body (`src/mcp/match.py` locally — `scripts/run_match.py` / `scripts/serve_match_http.py`; the cloud equivalent is account-gated, FR-CLOUD-5); the Gmail dimension consumes and sends; the Cop-only `send_final_report` tool fires exactly once.
...
- **OD6 (P8, ADR-D6-1):** RESOLVED — cloud platform = Render (Prefect Horizon evaluated first, reversed: its free tier forces platform OAuth in front of the app); whether the grader hits live URLs or only views screenshots (affects token-revoke timing).
