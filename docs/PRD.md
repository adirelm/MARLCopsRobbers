...
- Running the actual §9 **inter-group bonus match** (requires a partner `biu-rlNN` group; counts toward the **final project**, not A6). The bonus schema keys + interface contract are **specified** (FR-ANL-9 AC + PLAN P-bonus row); the serializer/spec **build is deferred to the final project**.
...
- **FR-ANL-9 (§9 bonus interface — SPECIFIED, build deferred; coordination dependency).** Specify the `bonus_report` JSON extension + the inter-group MCP contract (the cross-server tool contract a partner `biu-rlNN` group's opposing server must implement) — the AC below enumerates the keys. The serializer/spec **implementation is deferred to the final project** (the bonus counts there, not in A6); the actual match needs a partner group.
