...
- Running the actual §9 **inter-group bonus match** (counts toward the **final project**, not A6). The bonus serializer + schema + agreement/wire spec are now **BUILT** (`src/reporting/bonus.py`, `bonus_send.py`, `docs/schema/bonus.schema.json`, `docs/interfaces/intergroup_mcp.md` — FR-ANL-9); a partner group is confirmed and the **match itself is pending** joint scheduling + the rules-agreement freeze.
