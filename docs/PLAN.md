# PLAN — Assignment 6: MARL Cops & Robbers (adrl-001) — v1.2.0

> **Group code:** `adrl-001` · **Version:** `1.2.0` · **Solo submission** (work-streams WS1 / WS2, §10 R16)
...
| §9 bonus capability | Bonus report serializer + schema + inter-group agreement/wire spec **BUILT** (`src/reporting/bonus*.py`, `docs/schema/bonus.schema.json`, `docs/interfaces/intergroup_mcp.md`); **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a) | §9 |
...
| **P8** | Cloud-MCP | §8 4-stage runbook: (1) local `uv export` deploy deps (NO requirements.txt — uv-only); (2) `@mcp.tool` entrypoints load the committed actor-only checkpoint from `MODEL_PATH` (deploy/model/, OLoRA actor nets) + per-session GRU state (single worker, NOT stateless); (3) Render Blueprint deploy + capture public URLs; (4) confirm Gmail auto-report path. Swap to `JWTVerifier`+jti deny-list; pin fastmcp; deploy both entrypoints to Render with the SAME tool contract as Stage 1 (`request_move`/`reveal_location`/`query_opponent`/`new_sub_game`); wire peers; `smoke_cloud.py` (shared trace_id), `smoke_auth.py` (401 + revoke); `cloud_match.py`; `render.yaml` fallback pre-tested; `runbook.md`; redact before every screenshot | distributed match runs (≥1 valid cloud sub-game); cloud comms + 401 + revoke screenshots captured (cloud = upside, localhost canonical per ADR / R2) |
...
| **P-bonus** | §9 post-A6 | Inter-group interface spec + bonus report serializer — **BUILT** (`src/reporting/bonus.py` + `bonus_send.py`, `docs/schema/bonus.schema.json`, `docs/interfaces/intergroup_mcp.md`): `report_type:bonus_game`, `groups`, `students_group_1/2`, per-sub-game `cop_group`/`thief_group`, derived `totals_by_group` + `bonus_claim`, `mutual_agreement`; **off A6 critical path; **PLAYED and WON 2026-08-04** — `adrl-001` 60 — `biu-azri` 40, `bonus_claim` 10/7, `mutual_agreement: true`, both groups emailed their reports (README §9a)** | serializer + schema + spec present and tested; match DONE: 11-row agreement frozen, 6 valid sub-games played, drafts byte-compared via a canonical sha256, both groups emailed with `mutual_agreement:true` |
...
| Version starts at 1.00 | started `1.0.0` → `1.1.0` (L11 §5 Minimax-Q bonus) → now `1.2.0` (V3 breadth; `1.1.1` was the post-audit hardening — the §9 P-bonus phase is a SEPARATE track and is now complete) in `src/__init__.py` + `config.version` + `pyproject` | `tests/unit/test_config_loader.py` (see the implementation note above) |
...
| R2 | Cloud-MCP deploy (Render) cold-start / free-tier limit | M×H | §5.3 localhost-first is sufficient for grading comms; localhost logs = CANONICAL F4; warm-up ping + Render fallback pre-tested | submit local proof + cloud smoke proof; cloud is upside not gate |
