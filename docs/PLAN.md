...
| Version starts at 1.00 | started `1.0.0` → `1.1.0` (L11 §5 Minimax-Q bonus) → now `1.2.0` (post-audit hardening; distinct from the deferred §9 P-bonus phase) in `src/__init__.py` + `config.version` + `pyproject` | `tests/unit/test_config_loader.py` (see the implementation note above) |
